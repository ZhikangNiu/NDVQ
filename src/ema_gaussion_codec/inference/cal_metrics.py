...
def main():
    ...
    with open(f"{args.output_result_path}/pesq_scores.txt","w") as p, open(f"{args.output_result_path}/stoi_scores.txt","w") as s:
        for deg_wav_path in tqdm(list(Path(args.deg_dir).rglob(f'*.wav'))):
            relative_path = deg_wav_path.relative_to(args.deg_dir)
            ref_wav_path = Path(args.ref_dir) / relative_path.parents[0] /Path(deg_wav_path.name.replace(f'_bw{args.bandwidth}', '')).with_suffix(f'.{args.ext}')
