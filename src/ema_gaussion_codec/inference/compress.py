...
import binary
from ac import ArithmeticCoder, ArithmeticDecoder, build_stable_quantized_cdf

EncodedFrame = tp.Tuple[torch.Tensor, tp.Optional[torch.Tensor]]

def compress_to_file(model, wav: torch.Tensor, fo: tp.IO[bytes],
                     use_lm: bool = True):
    """Compress a waveform to a file-object using the given model.

    Args:
        model): a pre-traine to use to compress the audio.
...
def decompress_from_file(model,fo: tp.IO[bytes], device='cpu') -> tp.Tuple[torch.Tensor, int]:
...
def compress(model, wav: torch.Tensor, use_lm: bool = False) -> bytes:
...
def decompress(model,compressed: bytes, device='cuda') -> tp.Tuple[torch.Tensor, int]:
    """Decompress from a file-object.
    Returns a tuple `(wav, sample_rate)`.
